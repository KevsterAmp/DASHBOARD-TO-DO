...
//para sa edit thingy na di pa ayos kasi di nagloload correctly yung card!!! ayusin ko pa to mamaya
document.addEventListener("click", (e) => {
  if (e.target && e.target.classList.contains("edit-icon")) {
      const cardId = e.target.closest(".kanban-card").dataset.id;
      const selectedTask = notes.find((task) => task.id === cardId);
      if (selectedTask) {
          openEditPopup(selectedTask);
      }
  }
});
...
function openEditPopup(task) {
  ...
  const editTitleInput = editPopup.querySelector("#edit-title");
  const editDescTextarea = editPopup.querySelector("#edit-desc");
  editTitleInput.value = task.title;
  editDescTextarea.value = task.description;

  const tagRadioButtons = editPopup.querySelectorAll('input[name="edit-radio"]');
  tagRadioButtons.forEach((radio) => {
      if (radio.nextElementSibling.textContent.trim() === task.tag) {
          radio.checked = true;
      }
  });

  const updateTaskButton = editPopup.querySelector(".update-task");
    updateTaskButton.addEventListener("click", () => {
        
        task.title = editTitleInput.value;
        task.description = editDescTextarea.value;

        tagRadioButtons.forEach((radio) => {
            if (radio.checked) {
                task.tag = radio.nextElementSibling.textContent.trim();
                task.tagColor = tagColors[task.tag];
            }
        });
      ...
      const cardToUpdate = document.querySelector(`[data-id="${task.id}"]`);
      if (cardToUpdate) {
          cardToUpdate.querySelector("h4").textContent = task.title;
          cardToUpdate.querySelector("p").textContent = task.description;
          cardToUpdate.querySelector(".kanban-card-tag").textContent = task.tag;
          cardToUpdate.querySelector(".kanban-card-tag").style.backgroundColor = task.tagColor;
      }
  });
}

document.addEventListener("click", (e) => {
  if (e.target && e.target.classList.contains("close-edit-popup")) {
